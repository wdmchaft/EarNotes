...
@implementation QuizChordCreator
...
-(QuizSimpleQuestion *)nextSimpleQuestion {
	...
	int userChordsLevel = [[[SharedAppData sharedInstance] currentUser] chordsGuideModeLevel];
	
	int maxNumOfQuestions = [self getMaxNumberOfQuestionsForLevel: userChordsLevel];
	...
	QuizObjectsArrays *dataArrays = [[[SharedAppData sharedInstance] currentUser] chordsGuideMode];
	...
	for (NSInteger i = 0; i < answersNum; i++) {
		...
		// Escolher prioritariamente da fila 'learning' 
			...
			if ([dataArrays.learning count] < [self getMaxNumberOfLearningObjectsForLevel:userChordsLevel]) {
	...
	[Utils log:[NSString stringWithFormat: @"Size: %d", [answersArray count]]];
	// Baralhar as perguntas
	[answersArray shuffle];
	
	// Escolher a "resposta correcta"
	for (id d in answersArray) {
		[Utils log:[NSString stringWithFormat: @"Object :%@", d]];
	}
