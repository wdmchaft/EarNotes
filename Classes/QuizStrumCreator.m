...
-(QuizSimpleQuestion *)nextSimpleQuestion {
	...
	int userStrummingLevel = [[[SharedAppData sharedInstance] currentUser] strummingGuideModeLevel];
	
	int maxNumOfQuestions = [self getMaxNumberOfQuestionsForLevel: userStrummingLevel];
	...
	QuizObjectsArrays *dataArrays = [[[SharedAppData sharedInstance] currentUser] strummingGuideMode];
	
	NSMutableArray *answersArray = [[NSMutableArray alloc] init];
	
	// Se for uma pergunta sobre um som, limitar o número de respostas a 1
	if (question.questionType == QuestionTypeSingleSound) {
		answersNum = 1;
	} else {
		if (lastQuizObject != nil) {
			// Acrescentar a ultima resposta para nao ser repetida
			[answersArray addObject:lastQuizObject];
		}
	}
	...
	for (NSInteger i = 0; i < answersNum; i++) {
		...
		// Escolher prioritariamente da fila 'learning' 
			...
			if ([dataArrays.learning count] < [self getMaxNumberOfLearningObjectsForLevel:userStrummingLevel]) {
